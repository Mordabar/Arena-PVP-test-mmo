...
export function createCharacterFactory(Arena, scene, opts) {
  ...
  var nativeUalAllClasses = opts.nativeUalAllClasses === true;
  var directLibrary = null;
  /* v0.30: no runtime retarget/bake path remains. All classes use the native
     UAL skeleton; missing Source clips remain explicit blockers. */
  ...
  /* v0.30: el runtime nativo UAL no necesita generar ni subir las ~50 mallas
     procedurales de cuerpo/armaduras de la etapa legacy retarget body. Se conservan sólo
     como fallback de desarrollo si el renderer se crea sin `nativeUalAllClasses`. */
  if (!nativeUalAllClasses) {
  ...
  Character.prototype.applyPose = function (entity, pos, yaw, palette, fade, hurtTint) {
    if (this.usedGlb) {
      ...
      // v0.30: una sola inicialización de arma/escudo; no hay armadura procedural.
      this.buildGear(entity, palette);
      /* El maniquí procedural queda apagado: el cuerpo completo lo pone el GLB. */
    ...
    for (i = 0; i < this.pose.length; i++) {
      ...
      if (!mesh) {
        ...
        mesh.castShadow = this._shadowCasting;
